...
namespace MedioClinic.Controllers
{
    public class FormTestController : BaseController
    {
        ...
        [HttpPost]
        public ActionResult Index([System.Web.Http.FromBody] PageViewModel<FloridaEventRegistrationViewModel> uploadModel)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    FormItemRepository.InsertFormItem("BizForm.FloridaEventRegistration", uploadModel.Data);
                }
                catch
                {
